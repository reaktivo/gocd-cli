...
module.exports = {
  ...
  _mapPipelinesToChoices: function _mapPipelinesToChoices(pipelines, listOptions = {}) {
    const { showOnlyScheludable } = listOptions;
    return Promise.resolve(
      pipelines.map((pipeline, i) => (
        {
          name: pipeline.name,
          value: pipeline.value,
          disabled: i % 2 && 'Not scheludable'
        }
      ))
    );
  },
  ...
  pipeline: function pipeline(options, listOptions) {
    return this.group(options).then(options => {
      ...
      return this._mapPipelinesToChoices(group.pipelines, listOptions).then(choices => {
        return this.inquire(options, 'pipeline', choices);
      });
    });
  },
