...
module.exports = {
  ...
  _mapPipelinesToChoices: function _mapPipelinesToChoices(options, pipelines) {
    ...
    const promises = pipelines.map(pipeline => {
      // Should keep Status require here since if we dont
      // it won't work.
      ...
      return statusCmd.run(statusOptions)
      .then(status => (
        {
          name: pipeline.name,
          value: pipeline.name,
          disabled: !status[options.pipelineFilter] && 'Not available'
        }
      ));
    });
  ...
  session: function session(options) {
    const config = ini.read();
    if (!options.session && config.session) {
